...
What is the average time complexity of the primary lookup operation in CopyOnWriteArrayList?
- A) O(1)
- B) O(log n)
- C) O(n)
- D) O(n log n)
...
## Question 3: Memory
How much additional memory overhead does CopyOnWriteArrayList have per element (approximately)?

## Question 4: Concurrency
Is CopyOnWriteArrayList safe for concurrent access without external synchronization? Explain why or why not.

## Question 5: Scalability
How does CopyOnWriteArrayList perform as the number of elements grows from 10 to 10 million? What factors degrade performance?

## Question 6: Design
Why was CopyOnWriteArrayList designed this way? What alternatives were considered during its development?
...
Compare CopyOnWriteArrayList with the most similar alternative in the Java collections framework.
List 3 advantages and 3 disadvantages of each.

## Question 9: Real-World
Describe a real-world scenario where CopyOnWriteArrayList is the optimal choice.
