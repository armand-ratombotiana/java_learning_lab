# Security Implications of Java object layout and memory internals

## Security Boundaries

Java object layout and memory internals features interact with Java's security model in specific ways. Understanding these interactions helps developers write secure code.

## Threat Model

### Confidentiality
Improper Java object layout and memory internals usage can leak sensitive information. For example, incorrect memory management can leave sensitive data accessible. Thread-local storage can leak information between requests.
...
### Availability
Resource exhaustion attacks can target Java object layout and memory internals mechanisms. Thread pool exhaustion, memory exhaustion, and lock contention can all be exploited to degrade service availability.
...
Validate all inputs to Java object layout and memory internals mechanisms. Do not trust external data to determine resource allocation or locking behavior.
