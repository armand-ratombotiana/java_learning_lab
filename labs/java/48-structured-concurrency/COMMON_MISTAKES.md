# Common Mistakes in structured concurrency and scoped values
...
Improper error handling in structured concurrency and scoped values code can lead to subtle bugs. Common issues include catching exceptions too broadly, ignoring interrupt signals, and failing to restore state after errors.
...
Several performance antipatterns are common in structured concurrency and scoped values code. These include excessive synchronization, unnecessary allocations, and suboptimal data structure choices.
...
Using complex structured concurrency and scoped values features when simpler approaches would suffice is a common mistake. Over-engineering increases complexity without providing commensurate benefits.
...
Some structured concurrency and scoped values behaviors vary across platforms and JVM implementations. Code that works correctly on one platform may behave differently on another.
