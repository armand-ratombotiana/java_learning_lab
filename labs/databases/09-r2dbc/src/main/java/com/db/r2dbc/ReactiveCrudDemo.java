        ...
        Mono.from(factory.create())
            .flatMapMany(conn -> {
                ...
                return Flux.from(conn.createStatement("""
                    ...
                    .thenMany(
                        Flux.from(conn.createStatement("INSERT INTO products (name, price) VALUES ($1, $2)")
                    ...
                    .thenMany(
                        Flux.from(conn.createStatement("SELECT id, name, price FROM products ORDER BY price DESC")
                            .execute())
                            .flatMap(result ->
                                result.map((row, meta) -> {
                    ...
                    .then(
                        Flux.from(conn.createStatement("UPDATE products SET price = $1 WHERE name = $2")
                    ...
                    .then(
                        Flux.from(conn.createStatement("DELETE FROM products WHERE price < $1")
                            .bind("$1", 50.00)
                            .execute())
                            .flatMap(Result::getRowsUpdated)
                    ...
                    .thenMany(
                        Flux.from(conn.createStatement("SELECT COUNT(*) AS cnt FROM products")
                            .execute())
