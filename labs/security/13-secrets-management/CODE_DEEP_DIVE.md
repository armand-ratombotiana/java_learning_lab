# Code Deep Dive: 13-secrets-management

## VaultSecretManager.java

### Purpose
Manages secrets lifecycle with HashiCorp Vault integration, including authentication, secret retrieval, and rotation.

### Key Implementation Details

**Vault Authentication Flow**:
1. Client authenticates via AppRole (RoleID + SecretID)
2. Vault returns client token with TTL
3. Token cached with automatic renewal before expiry
4. Token wrapped in VaultTemplate for API calls

### Class Structure

The VaultSecretManager implements:
- uthenticate() - AppRole or Kubernetes auth
- getSecret(String path) - Retrieve secret from KV engine
- otateSecret(String path) - Generate new secret version
- listSecrets(String path) - Enumerate secret paths

### Secret Resolution Order

1. Vault (primary)
2. Environment variables (fallback)
3. Encrypted config file (development only)
4. Application.properties (defaults, no secrets)

### Spring Vault Integration

`java
@VaultPropertySource(value = "secret/application", renewal = NEVER)
@Configuration
...
- Token TTL: Auto-renewal at 50% TTL
- TLS: Required for all Vault communication
- Audit: All secret access logged
- Rotation: Scheduled rotation with grace period
- Failure mode: Fail secure (deny access, not default to less secure)
